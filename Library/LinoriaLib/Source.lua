...
function Library:Notify(Text, Time, SoundId, Volume)
    local Side = string.lower(Library.NotifySide);
    if Side == "right" then
        Library:RightNotify(Text, Time, SoundId, Volume);
    else
        Library:LeftNotify(Text, Time, SoundId, Volume);
    end
end;

function Library:LeftNotify(Text, Time, SoundId, Volume)
...
function Library:RightNotify(Text, Time, SoundId, Volume)
    local XSize, YSize = Library:GetTextBounds(Text, Library.Font, 14);
...
function Library:CreateWindow(...)
    ...
    if typeof(Config.AutoLock) ~= 'boolean' then Library.AutoLock = false else Library.AutoLock = Config.AutoLock end
